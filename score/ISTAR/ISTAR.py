...
class IStarIRC3(IconScoreBase):
    ...
    def __init__(self, db: IconScoreDatabase) -> None:
        ...
        ## auction storage DB 
        self._auction = ArrayDB("auction", db, value_type=strt)
...
@external
def auction_sell(self)
    ## make sell model (msg.sender,tokenId,property,icx)
    json_sell = {}
    json_sell['address'] = self.msg.sender
    json_sell['tokenId'] = self.msg.sender
    # json_sell['property'] = self.msg.sender.tokenId[property]
    # json_sell['icx'] = 

    self._auction.put(str(json_result))
...
def auction_results(self) -> dict:
    ...
    for value in self._auction:
        acutionList.append(value)

    return {'result': acutionList}
